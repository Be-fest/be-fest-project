...
SELECT 
  ...
  -- Diferença:
  es.total_estimated_price - (
    (e.full_guests * es.price_per_guest_at_booking) + 
    (e.half_guests * (es.price_per_guest_at_booking / 2))
  ) as diferenca
...
SET total_estimated_price = (
  (e.full_guests * event_services.price_per_guest_at_booking) + 
  (e.half_guests * (event_services.price_per_guest_at_booking / 2))
)
FROM events e
WHERE event_services.event_id = e.id
  ...
  AND event_services.total_estimated_price != (
    (e.full_guests * event_services.price_per_guest_at_booking) + 
    (e.half_guests * (event_services.price_per_guest_at_booking / 2))
  );
...
SELECT 
  ...
  e.full_guests,
  e.half_guests,
  -- Cálculo para confirmar:
  (e.full_guests * es.price_per_guest_at_booking) + 
  (e.half_guests * (es.price_per_guest_at_booking / 2)) as confirmacao
FROM event_services es
JOIN events e ON es.event_id = e.id
WHERE es.price_per_guest_at_booking IS NOT NULL
  AND es.total_estimated_price IS NOT NULL
